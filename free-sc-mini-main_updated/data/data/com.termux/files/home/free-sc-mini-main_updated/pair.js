...
const config = {
  ...
  FREE_IMAGE: 'https://files.catbox.moe/rofxqa.jpg', // Harmonized by Gemini
  ...
  IMAGE_PATH: 'https://files.catbox.moe/rofxqa.jpg', // Updated by Gemini
...
🎯 ᴛᴀᴘ ᴀ ᴄᴀᴛᴇɢᴏʀʏ ʙᴇʟᴏᴡ!`;
    ...
    const buttons = [
      { buttonId: `${config.PREFIX}generalcommands`, buttonText: { displayText: "ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs" }, type: 1 },
...
case 'play': {
  ...
  try {
    ...
    let audioBuffer;
    let songTitle;
    let thumbnailUrl; // To store the thumbnail URL
    ...
    if (match) {
        const videoId = match[1];
        await socket.sendMessage(sender, { text: `*Fetching audio from YouTube video...*` }, { quoted: msg });
        const ytmp3 = require('denethdev-ytmp3');
        ...
        songTitle = audioData.title || 'Unknown Track';
        thumbnailUrl = audioData.thumbnail; // Assuming ytmp3 returns thumbnail
    } else {
        await socket.sendMessage(sender, { text: `*Searching for "${query}" on YouTube...*` }, { quoted: msg });
        const yts = require('yt-search');
        const r = await yts(query);
        ...
        const firstVideo = videos[0];
        await socket.sendMessage(sender, { text: `*Fetching audio for "${firstVideo.title}"...*` }, { quoted: msg });
        const ytmp3 = require('denethdev-ytmp3');
        ...
        thumbnailUrl = firstVideo.thumbnail; // Assuming yt-search returns thumbnail
    }
    ...
    if (thumbnailUrl) {
        try {
            ...
            console.warn('Failed to fetch thumbnail:', thumbError.message);
        }
    }
