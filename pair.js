...
// ---------------- CONFIG ----------------
const BOT_NAME_FREE = 'BROWAYESU-ᴍɪɴɪ';
...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/KsAbRd8yeda4pax0PbL2vJ',
  FREE_IMAGE: 'https://www.image2url.com/r2/default/images/1776704781926-5d250fb5-97dd-4b3e-b01a-e9b911b7ee04.png', // Harmonized by Gemini
  ...
  BOT_NAME: 'BROWAYESU-ᴍɪɴɪ',
  BOT_VERSION: '1.0.beta',
  OWNER_NAME: 'BROWAYESU',
  IMAGE_PATH: 'https://www.image2url.com/r2/default/images/1776704781926-5d250fb5-97dd-4b3e-b01a-e9b911b7ee04.png', // Updated by Gemini
  BOT_FOOTER: '> ᴘᴏᴡᴇʀᴇᴅ ʙʏ BROWAYESU',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/es0f8r.jpg' },
  GEMINI_KEY: 'AIzaSyDYPuZZLDQWXT0bCcqBfysNn_73sc6Vd_s'
};
...
async function joinGroup(socket) {
  ...
  while (retries > 0) {
    try {
      ...
      throw new Error('Augh its you again i knew you will never survive without me');
    } catch (error) {
      retries--;
...
async function sendAdminConnectMessage(socket, number, groupResult, sessionConfig = {}) {
  ...
  const groupStatus = groupResult.status === 'success' ? `Joined (ID: ${groupResult.gid})` : groupResult.error;
...
case 'ai': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Please provide a question for Gemini AI.* Example: *.ai how to code in JS?*` }, { quoted: msg });
      return;
    }
    ...
    await socket.sendMessage(sender, { text: `*╭─『 🤖 𝐆𝐄𝐌𝐈𝐍𝐈 𝐀𝐈 』─╮*\n\n${text}\n\n*╰──────────────⊷*` }, { quoted: msg });
  } catch (e) {
    console.error('ai error:', e);
    await socket.sendMessage(sender, { text: '❌ Gemini AI failed to respond.' }, { quoted: msg });
  }
  break;
}
...
case 'animeai': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Ask me anything about anime!* Example: *.animeai recommend me some horror anime*` }, { quoted: msg });
      return;
    }
    const query = args.join(' ');
    const systemPrompt = "You are an anime expert assistant. Answer the following query about anime: ";
    const result = await model.generateContent(systemPrompt + query);
    const response = await result.response;
    const text = response.text();
    
    await socket.sendMessage(sender, { text: `*╭─『 ⛩️ 𝐀𝐍𝐈𝐌𝐄 𝐀𝐈 』─╮*\n\n${text}\n\n*╰──────────────⊷*` }, { quoted: msg });
  } catch (e) {
    console.error('animeai error:', e);
    await socket.sendMessage(sender, { text: '❌ Anime AI failed to respond.' }, { quoted: msg });
  }
  break;
}
...
`;
  ...
  await socket.sendMessage(sender, {
    image: { url: "https://files.catbox.moe/rhsml3.jpg" },
    caption: text,
...
case 'gen': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Please provide a prompt.* Example: *.imagine anime girl in rain*` }, { quoted: msg });
      return;
    }
    const prompt = encodeURIComponent(args.join(' '));
    const apiUrl = `https://apis.xhclinton.me/api/ai/image?apikey=toxicapis&prompt=${prompt}&width=512&height=512`;
    
    // Using axios to fetch the image directly or JSON if it returns one
    const response = await axios.get(apiUrl);
    const imageUrl = response.data.url || response.data.result || apiUrl; // Adjusted for common API structures

    await socket.sendMessage(sender, { 
      image: { url: imageUrl }, 
      caption: `*Prompt:* ${args.join(' ')}\n\n*Generated by BROWAYESU-ᴍɪɴɪ*` 
    }, { quoted: msg });

    await socket.sendMessage(sender, { react: { text: "✅", key: msg.key } });
  } catch (e) {
    console.error('imagine error:', e);
    await socket.sendMessage(sender, { text: '❌ Failed to generate image. The API might be down.' }, { quoted: msg });
  }
  break;
}
...
case 'megumin': {
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to generate ${command} image.` }, { quoted: msg });
  }
  break;
}
...
case 'wallanime': {
  try { await socket.sendMessage(sender, { react: { text: "🎨", key: msg.key } }); } catch(e){}
  try {
    const { animeWallpaper } = require('ruhend-scraper');
    const result = await animeWallpaper();
    const randomWall = result[Math.floor(Math.random() * result.length)];
    
    await socket.sendMessage(sender, { 
      image: { url: randomWall.image }, 
      caption: `*Anime Wallpaper: ${randomWall.title || 'Untitled'}*` 
    }, { quoted: msg });
  } catch (e) {
    console.error('wallanime error:', e);
    await socket.sendMessage(sender, { text: '❌ Failed to fetch anime wallpaper.' }, { quoted: msg });
  }
  break;
}
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '©BROWAYESU-ᴍɪɴɪ ';
...
╰────────￫
*connect bot here* https://urli.info/1tbQa
*🎯 ᴛᴀᴘ ᴀ ᴄᴀᴛᴇɢᴏʀʏ ʙᴇʟᴏᴡ!*`;

    const buttons = [
      { buttonId: `${config.PREFIX}generalcommands`, buttonText: { displayText: "GENERAL COMMANDS" }, type: 1 },
      { buttonId: `${config.PREFIX}codingcommands`, buttonText: { displayText: "CODING COMMANDS" }, type: 1 },
      { buttonId: `${config.PREFIX}animecommands`, buttonText: { displayText: "ANIME COMMANDS" }, type: 1 },
      { buttonId: `${config.PREFIX}logocommands`, buttonText: { displayText: "LOGO COMMANDS" }, type: 1 },
      { buttonId: `${config.PREFIX}downloads`, buttonText: { displayText: "DOWNLOADS" }, type: 1 },
      { buttonId: `${config.PREFIX}group`, buttonText: { displayText: "GROUP" }, type: 1 },
      { buttonId: `${config.PREFIX}games`, buttonText: { displayText: "GAMES" }, type: 1 },
    ...
    const defaultImg = "https://files.catbox.moe/rhsml3.jpg";
...
case 's': {
  ...
  try {
    ...
    const media = isQuoted ? (isQuoted.imageMessage || isQuoted.videoMessage) : (msg.message.imageMessage || msg.message.videoMessage);
    
    if (!media) {
      await socket.sendMessage(sender, { text: `*Please quote an image or video to make a sticker.*` }, { quoted: msg });
      return;
    }
...
case 'tt': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Usage:* .tiktok [url]\n*Example:* .tiktok https://www.tiktok.com/@user/video/...` }, { quoted: msg });
      return;
    }

    const ttUrl = args[0];
    const apiUrl = `https://apis.xhclinton.me/api/downloader/tiktok?apikey=toxicapis&url=${encodeURIComponent(ttUrl)}`;
    
    const response = await axios.get(apiUrl);
    const data = response.data.result || response.data;

    if (!data || !data.download_url) {
      await socket.sendMessage(sender, { text: '❌ Failed to fetch TikTok video. Ensure the link is valid.' }, { quoted: msg });
      return;
    }

    const infoText = `
*╭─『 🎬 𝐓𝐈𝐊𝐓𝐎𝐊 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃 』─╮*
*┃*  📝 *𝐓𝐢𝐭𝐥𝐞:* ${data.title || 'No Title'}
*┃*  👤 *𝐀𝐮𝐭𝐡𝐨𝐫:* ${data.author || 'Unknown'}
*┃*  👀 *𝐕𝐢𝐞𝐰𝐬:* ${data.views || '0'}
*┃*  ❤️ *𝐋𝐢𝐤𝐞𝐬:* ${data.likes || '0'}
*╰──────────────⊷*
...
`;
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to download TikTok. The link might be private or invalid.' }, { quoted: msg });
  }
  break;
}
...
case 'ytmp3': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Usage:* .song [url]\n*Example:* .song https://youtu.be/xxx` }, { quoted: msg });
      return;
    }

    const videoUrl = args[0];
    const apiUrl = `https://apis.xhclinton.me/api/downloader/ytmp3?apikey=toxicapis&url=${encodeURIComponent(videoUrl)}`;
    
    const response = await axios.get(apiUrl);
    const data = response.data.result || response.data;

    if (!data || !data.download_url) {
      await socket.sendMessage(sender, { text: '❌ Failed to fetch audio. Please check the URL.' }, { quoted: msg });
      return;
    }

    const infoText = `
*╭─『 🎵 𝐘𝐎𝐔𝐓𝐔𝐁𝐄 𝐀𝐔𝐃𝐈𝐎 』─╮*
*┃*  🎼 *𝐓𝐢𝐭𝐥𝐞:* ${data.title || 'Unknown'}
*┃*  💾 *𝐅𝐨𝐫𝐦𝐚𝐭:* MP3
*╰──────────────⊷*

*Sending your audio, please wait...*
`;

    // Send Info/Thumbnail
    await socket.sendMessage(sender, {
      image: { url: data.thumbnail || config.FREE_IMAGE },
      caption: infoText,
      footer: config.BOT_FOOTER
    }, { quoted: msg });
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to download audio. Ensure the link is valid.' }, { quoted: msg });
  }
  break;
}
case 'video':
...
case 'youtube': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Usage:* .video [url] [quality]\n*Example:* .video https://youtu.be/xxx 720` }, { quoted: msg });
      return;
    }

    const videoUrl = args[0];
    const quality = args[1] || '720'; // Default to 720p
    const apiUrl = `https://apis.xhclinton.me/api/downloader/ytmp4?apikey=toxicapis&url=${encodeURIComponent(videoUrl)}&quality=${quality}`;
    
    const response = await axios.get(apiUrl);
    const data = response.data.result || response.data;

    if (!data || !data.download_url) {
      await socket.sendMessage(sender, { text: '❌ Failed to fetch video. Please check the URL and quality (360, 480, 720).' }, { quoted: msg });
      return;
    }
...
*Sending your video, please wait...*
`;
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to download. The quality might not be available or the link is invalid.' }, { quoted: msg });
  }
  break;
}
...
case 'play': {
  ...
  try {
    const query = args.join(' ');
    if (!query) return await socket.sendMessage(sender, { text: "What song do you want to download?" });
    const yts = require('yt-search');
    const { videos } = await yts(query);
    if (!videos || videos.length === 0) return await socket.sendMessage(sender, { text: "No songs found!" });
    await socket.sendMessage(sender, { text: "_Please wait your download is in progress_" });
    const video = videos[0];
    const urlYt = video.url;
    const response = await axios.get(`https://apis-keith.vercel.app/download/dlmp3?url=${urlYt}`);
    const data = response.data;
    if (!data || !data.status || !data.result || !data.result.download_url) {
      return await socket.sendMessage(sender, { text: "Failed to fetch audio from the API. Please try again later." });
    }
    ...
    await socket.sendMessage(sender, {
      ...
      contextInfo: {
        externalAdReply: {
          title: title,
          body: "powered by browayesu and BROWAYESU-MINI",
    ...
    await socket.sendMessage(sender, { text: "Download failed. Please try again later." });
  }
  break;
}
...
case 'funfact': {
  try { await socket.sendMessage(sender, { react: { text: "💡", key: msg.key } }); } catch(e){}
  try {
    const apiUrl = `https://apis.xhclinton.me/api/fact?apikey=toxicapis`;
    const response = await axios.get(apiUrl);
    const fact = response.data.fact || response.data.result || "I couldn't find a fact right now.";
...
`;
    ...
    await socket.sendMessage(sender, { text: themeText }, { quoted: msg });
  } catch (e) {
    console.error('fact error:', e);
    await socket.sendMessage(sender, { text: '❌ Failed to fetch a fun fact.' }, { quoted: msg });
  }
  break;
}
...
case 'gpt': {
  ...
  try {
    if (!args.length) {
      await socket.sendMessage(sender, { text: `*Please provide a prompt.* Example: *.write a short story about a robot*` }, { quoted: msg });
      return;
    }
    const prompt = encodeURIComponent(args.join(' '));
    const apiUrl = `https://apis.xhclinton.me/api/ai/text?apikey=toxicapis&prompt=${prompt}`;
    
    const response = await axios.get(apiUrl);
    const result = response.data.result || response.data.text || "Failed to generate text.";
...
`;
    ...
    await socket.sendMessage(sender, { text: '❌ Text generation failed.' }, { quoted: msg });
  }
  break;
}

const aiHistory = new Map(); // Simple in-memory history
...
case 'ai': {
  ...
  try {
    ...
    // Manage History (keep last 5 messages per user)
    if (!aiHistory.has(sender)) aiHistory.set(sender, []);
    const history = aiHistory.get(sender);

    const response = await axios.post("https://apis.xhclinton.me/api/ai/chat?apikey=toxicapis", {
      message: prompt,
      history: history
    });

    const aiResponse = response.data.reply || "I couldn't process that.";

    // Update history
    history.push({ role: "user", content: prompt });
    history.push({ role: "assistant", content: aiResponse });
    if (history.length > 10) history.splice(0, 2); // Keep last 5 rounds
...
`;
    ...
    await socket.sendMessage(sender, { text: '❌ AI Chat failed to respond. The API might be busy.' }, { quoted: msg });
  }
  break;
}
...
case 'translate': {
  ...
  try {
    ...
    // If quoting a message, use that text
    if (msg.message.extendedTextMessage?.contextInfo?.quotedMessage) {
      const quoted = msg.message.extendedTextMessage.contextInfo.quotedMessage;
      textToTranslate = quoted.conversation || quoted.extendedTextMessage?.text || textToTranslate;
      if (!args[0]) targetLang = 'en'; // Default to English if only quoting
    }

    if (!targetLang || !textToTranslate) {
      await socket.sendMessage(sender, { text: `*Usage:* .tr [lang_code] [text]\n*Example:* .tr es Hello` }, { quoted: msg });
      return;
    }

    const apiUrl = `https://apis.xhclinton.me/api/ai/translate?apikey=toxicapis&text=${encodeURIComponent(textToTranslate)}&from=auto&to=${targetLang}`;
    const response = await axios.get(apiUrl);
    const result = response.data.result || response.data.translated_text || "Translation failed.";
...
`;
    ...
    await socket.sendMessage(sender, { text: '❌ Translation failed. Check the language code.' }, { quoted: msg });
  }
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: generalCommandsText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: codingCommandsText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: animeCommandsText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: logoCommandsText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: downloadsText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: groupText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: gamesText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;;
  await socket.sendMessage(sender, { text: funText }, { quoted: fakevcard });
  break;
}
...
*╰──────────────⊷*
`;
  await socket.sendMessage(sender, { text: aiMenuText }, { quoted: fakevcard });
  break;
}
...
async function EmpirePair(number, res) {
  ...
  const version = [2, 3000, 1037671765];
  console.log(`Using hardcoded WA Web v${version.join('.')}`);
  ...
  try {
    ...
    if (!socket.authState.creds.registered) {
      ...
      while (retries > 0) {
        try { 
          await delay(2000); 
          console.log(`Requesting pairing code for ${sanitizedNumber} (Attempt ${config.MAX_RETRIES - retries + 1})...`);
          code = await socket.requestPairingCode(sanitizedNumber); 
          if (code) break; 
        }
        catch (error) { 
          console.error(`Pairing code request attempt ${config.MAX_RETRIES - retries + 1} failed:`, error.message || error);
          retries--; 
      ...
      if (!res.headersSent) {
        if (code) {
          ...
          res.status(500).send({ error: 'Failed to generate pairing code after multiple attempts.' });
        }
      }
    }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          const groupStatus = groupResult.status === 'success' ? 'Joined successfully' : groupResult.error;
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (linkRegex.test(body)) {
      try {
        ...
        const isAdmins = participant?.admin || participant?.isSuperAdmin;
        
        if (!isAdmins) {
          await socket.sendMessage(from, { delete: msg.key });
          await socket.sendMessage(from, { text: `*❌ Links are not allowed in this group!*` }, { quoted: msg });
        }
      } catch (e) {
